...
interface ProseEditorProps {
    ...
    initialHtml?: string; // Payload from history
    documentId?: string;  // Active document signature
}
...
export default function ProseEditor({ blocks, outlineData, initialHtml, documentId }: ProseEditorProps) {
    ...
    /**
     * Resolves the active DOM state based on whether it's a fresh generation (blocks) 
     * or a restored document from the vault (initialHtml).
     */
    ...
    // Resilient Meta Tag Extraction
    const metaTitle = (outlineData as any).title || outlineData.headings?.[0]?.text || "Generated SEO Article";

    const extractDescription = () => {
        if (blocks && blocks.length > 0) {
            return blocks.find(b => b.type === 'paragraph')?.content?.replace(/<[^>]*>?/gm, '') || "";
        }
        if (initialHtml) {
            const match = initialHtml.match(/<p>(.*?)<\/p>/);
            if (match && match[1]) return match[1].replace(/<[^>]*>?/gm, '');
        }
        return "Optimized document generated by ContentForge AI.";
    };
    const metaDescription = extractDescription().substring(0, 155) + "...";
    ...
    useEffect(() => {
        ...
        const saveToDatabase = async () => {
            // Bypass autosave if viewing an existing document to prevent duplication
            if (documentId) {
