...
export default function ProseEditor({ blocks, outlineData, initialHtml, documentId }: ProseEditorProps) {
    ...
    const generateHTMLFromBlocks = useCallback(() => {
        ...
        const rawHtml = blocks.map(block => {
            ...
            if (block.type === 'paragraph' && typeof block.content === 'string') return `<p>${block.content}</p>`;
    ...
    // Parse SEO Metadata from blocks payload on mount
    useEffect(() => {
            ...
            try {
                ...
                const response = await fetch('/api/documents/save', {
                    ...
                    body: JSON.stringify({
                        ...
                        seoMetadata: seoMeta // Pass editable meta back to DB
                    })
                });
    ...
    }, [seoMeta.metaTitle]); // Trigger re-save if title significantly changes (optional architecture decision)
        ...
        try {
            ...
            const response = await fetch('/api/documents/publish', {
                ...
                body: JSON.stringify({
                    ...
                    seoMetadata: seoMeta // Includes RankMath target keyword etc.
                })
            });
