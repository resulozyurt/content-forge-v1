...
import { ShieldCheck, Users, Zap, ArrowUpCircle, Loader2, AlertTriangle, ShieldAlert } from "lucide-react";
...
export default function AdminDashboardPage() {
    ...
    // Translation matrix initialization. Ensure 'Admin' key exists in your en.json and tr.json
    ...
    // Enforce rigorous client-side administrative shielding
    useEffect(() => {
    ...
    // Dispatch administrative override commands to the backend
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex items-center gap-3 bg-gray-900 border border-gray-800 rounded-xl px-4 py-2 shadow-sm">
                    <Users className="w-5 h-5 text-gray-400" />
                    <div>
                        <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">{t('totalUsers')}</p>
            ...
            <div className="bg-[#0a0a0a] border border-gray-800 rounded-2xl shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-gray-900/50 text-gray-400 uppercase text-xs font-bold tracking-widest border-b border-gray-800">
                            <tr>
                                <th className="px-6 py-4">{t('accountOrigin')}</th>
                                <th className="px-6 py-4">{t('securityPrivilege')}</th>
                                <th className="px-6 py-4">{t('networkStatus')}</th>
                                <th className="px-6 py-4">{t('tokenLedger')}</th>
                                <th className="px-6 py-4 text-right">{t('actions')}</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-800/50">
                            {users.map((user) => (
                                <tr key={user.id} className="hover:bg-gray-900 transition-colors group">
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col">
                                            <span className="font-bold text-white">{user.email}</span>
                                            <span className="text-xs text-gray-500 mt-0.5">{t('joined')}: {new Date(user.createdAt).toLocaleDateString()}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        {user.isVerified ? (
                                            <span className="text-green-500 font-bold text-xs flex items-center gap-1.5">
                                                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div> {t('verified')}
                                            </span>
                                        ) : (
                                            <span className="text-amber-500 font-bold text-xs flex items-center gap-1.5">
                                                <AlertTriangle className="w-3 h-3" /> {t('pendingOtp')}
                                            </span>
                                        )}
                                    </td>
